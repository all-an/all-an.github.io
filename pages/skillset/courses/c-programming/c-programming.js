// Renders the C Programming courses list. The dataset (DATA, catLabel,
// catClass, statusLabel, statusClass) comes from c-programming-data.js,
// which is loaded first. A course name links to its local detail page when
// one exists (the `page` field), otherwise out to its course/certificate
// when a real url is set (anything other than "#").

// A course's detail page can override its status via "Mark done" / "Mark
// planned" buttons, saved to localStorage under 'course-status:<folder>'.
// This reads that override when present, falling back to the dataset value.
